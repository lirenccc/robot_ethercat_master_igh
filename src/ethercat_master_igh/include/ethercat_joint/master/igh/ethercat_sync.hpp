 ...
 */
 ...
 public:
     ...
     * 用于 clock_nanosleep（Job 线程必须调用，否则 PLL 补偿为死代码）
     */
    struct timespec getSleepSpec(uint64_t wakeup_time_ns) const;
    ...
    /**
     * @brief 兼容入口（天机参考实现无门控；读回始终允许，本接口仅作观测钩子）。
     * @note 不禁止 DC 同步数据报发送；也不再门控 reference_clock_time 读回。
     */
    ...
    bool isDcPllActive() const {
        return dc_started_ && !ref_clock_read_disabled_;
    }
 ...
 private:
    ...
    static constexpr int kRefClockFailEnterBackoff = 12;
    static constexpr uint64_t kRefClockBackoffEveryCycles = 2500;  // ~5s @2ms
    static constexpr uint64_t kRefClockBackoffMaxCycles = 25000;   // ~50s @2ms
    int warmup_cycles_remaining_ = kRefClockWarmupCycles;
    int consecutive_ref_successes_ = 0;
    int consecutive_ref_failures_ = 0;
    uint64_t ref_clock_process_cycle_ = 0;
    int ref_clock_fail_streak_ = 0;
    bool ref_clock_backoff_ = false;
    int32_t last_good_dc_diff_ns_ = 0;
    bool have_good_dc_diff_ = false;
